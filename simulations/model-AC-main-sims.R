...
# Code: script to run simulations for Model AC in Doser et al. (2020). 
#       Model AC incorporates automated acoustic vocalization data and 
#       point count data. Abundance is varying across space.
...
for (i in 1:n.sims) {
  ...
  if (i %in% c(5, 10, 30)) {
    date <- Sys.Date()
    file.name <- paste('../../results/covariate-model-sim-results-model-3-', i, '-simulations-', date, '.R', sep = '')
